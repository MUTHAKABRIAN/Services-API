...
public class Sql2oElectricianDao implements ElectricianDao {
    ...
    public void Add(Electrician electrician) {
        String sql = "INSERT INTO electrician (electricianId, electrician_name,image,physical_address,email,rating, description) VALUES (:electricianId, :electrician_name,:image,:physical_address,:email,:rating, :description)";
        try (Connection conn = sql2o.open()) {
            int id = (int) conn.createQuery(sql, true)
                    .bind(electrician)
                    .executeUpdate()
                    .getKey();
            electrician.setElectricianId(id);
